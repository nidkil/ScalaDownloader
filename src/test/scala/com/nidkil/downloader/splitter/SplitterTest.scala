...
class SplitterTest extends FunSpec with Matchers {
  ...
  describe("A DefaultSplitter") {
    it("should return 1 chunk when the default splitter used") {
    ...
    it("should return 5 chunk when the default splitter used") {
    ...
    it("should return 10 chunks when the standard ratio strategy is used") {
    ...
    it("should return 20 chunks when the standard ratio with max size strategy is used") {
    ...
    it("should return 4 chunks when a custom ratio splitter is used") {
      val info = new RemoteFileInfo(url, "application/x-compressed", true, 1024 * 1024 * 5, new Date())
      val splitter = new Splitter()
      val chunks = splitter.split(info, true, workDir, customStrategy)
      assert(chunks.size == 4)
    }
    it("should return 4 chunks when a custom ratio splitter is used, last chunk should be smaller than 1 MB") {
